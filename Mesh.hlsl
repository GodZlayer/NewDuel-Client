...
PS_INPUT VS_Skin(VS_SKIN_INPUT input)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        pos.xyz += mul(BoneMatrices[idx], float4(input.pos, 1.0f)).xyz * weight;
        normal += mul((float3x3)BoneMatrices[idx], input.normal) * weight;
    }

    PS_INPUT output;
    output.pos = mul(WorldViewProj, float4(pos.xyz, 1.0f));
    output.color = input.color;
    output.tex0 = input.tex0;
